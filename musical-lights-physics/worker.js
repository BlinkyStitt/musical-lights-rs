...
const ready = init().then(instance => {
  ...
  impulseTotals = new Float32Array(layout[12] - layout[10]);
});
...
function publish() {
  ...
  postMessage({ type: 'snapshot', buffer, debt, maxDebt, steps, totalCost,
    tick: simulation.tick(), timestamp: absoluteNow(), sequence: request.sequence }, [buffer]);
  buffer = null;
  request = null;
}
    ...
    switch (data.type) {
      ...
      case 'report':
        postMessage({ type: 'report', inputs: recording, physicsCosts: Array.from(costs.subarray(0, costCount)),
          recordingOverflow, config: Array.from(config), initialTick, finalTick: simulation.tick(),
