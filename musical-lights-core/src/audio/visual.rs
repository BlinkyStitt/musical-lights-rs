...
pub struct VisualLevels {
    pub bands: [f32; DISPLAY_BANDS],
    pub panel_rows: [f32; PANEL_ROWS],
}
...
impl<const N: usize> DisplaySnapshot<N> {
    ...
    pub fn push(&mut self, at: f64, values: [f32; N], reduced_motion: bool) {
        self.advance(at);
        self.reduced_motion = reduced_motion;
        for (band, value) in self.bands.iter_mut().zip(values) {
            band.target = value;
            if value > band.height {
                band.height = value;
                band.velocity = 0.0;
    ...
    fn advance(&mut self, at: f64) {
        ...
        for band in &mut self.bands {
            if at > band.hold_until {
                ...
                let distance = (band.height - band.target).max(0.0);
                ...
                band.height = band.target + remaining;
                band.velocity = (velocity + rate * coefficient * elapsed) * decay;
                if remaining <= 0.0001 {
                    band.height = band.target;
                    band.velocity = 0.0;
                }
    ...
    /// hold deadline, edge opacity, current target. No PCM crosses this boundary.
    pub fn write_transport(&self, output: &mut [f64]) {
        assert_eq!(output.len(), 2 + N * 5);
        output[0] = self.at;
...
mod tests {
    ...
    fn short_tap_survives_a_large_audio_block_and_render_stalls_do_not_replay_it() {
        ...
        state.push(0.100, [0.8; 24], false);
        state.push(0.110, [0.0; 24], false);
    ...
    fn render_sampling_cannot_change_the_motion_or_hold_deadline() {
        for reduced in [false, true] {
            ...
            state.push(0.0, [1.0], reduced);
            state.push(0.010, [0.0], reduced);
    ...
    fn transport_preserves_motion_and_rejects_corrupt_state() {
        let mut state = DisplaySnapshot::<24>::new(0.0);
        state.push(1.0, [0.7; 24], true);
        state.push(1.5, [0.1; 24], true);
        let mut wire = [0.0; 122];
        state.write_transport(&mut wire);
        let decoded = DisplaySnapshot::<24>::from_transport(&wire).unwrap();
    ...
    fn shared_gain_preserves_sustained_activity_and_does_not_modify_loudness() {
        ...
        assert!((values.bands[0] - 0.8).abs() < 1e-6);
        assert!((values.bands[1] - 2.0 / 3.0).abs() < 1e-6);
        assert!((values.panel_rows[0] - 6.0 / 7.0).abs() < 1e-6);
        ...
        assert_eq!(gain.map(&silence).bands, [0.0; 24]);
            ...
            for period_ms in [20, 80, 150, 250, 350, 500, 800] {
                for plot_luminance in [0.01_f32, 0.92] {
                    ...
                    for ms in (0..6000).step_by(2) {
                        ...
                        display.push(ms as f64 / 1000.0, [input; DISPLAY_BANDS], reduced_motion);
    ...
    fn steady_sound_fades_its_edge_without_retriggering() {
        let mut state = DisplaySnapshot::<24>::new(0.0);
        for step in 0..2500 {
            state.push(step as f64 * 0.002, [0.5; 24], false);
        }
