...
Fullscreen now expands the same graph into a lights-only page view on every browser. Where supported, the native API also hides browser chrome. The expanded view keeps an Exit button and hides the surrounding page and secondary controls. On iPhone Safari, the page cannot remove browser chrome through the unsupported element Fullscreen API; see [WebKit issue 206854](https://bugs.webkit.org/show_bug.cgi?id=206854). The published site also passed the iPhone WebKit check with 24 live bands, fullscreen entry and exit, uninterrupted audio, and capture cleanup. The synthetic microphone source uses the `MediaDevices` prototype because an instance override did not survive the live page's load in WebKit. These WebKit tests run on the Mac. They do not verify a physical iPhone, Safari's address bar, or device safe-area insets.
