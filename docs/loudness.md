...
The browser displays 240 loudness samples across the 24-Bark scale, spaced at 0.1 Bark. These are samples of the model's loudness curve, not independent critical bands or auditory channels. The browser uses density directly for artistic height: `min(1.25 * gain * density / (1 + gain * density), 1)`. It keeps measured sample loudness as `density * 0.1` sones. All samples share the existing aggregate-driven gain; it advances once per loudness frame. Terminal, LED, and sphere-physics values keep their aggregate mapping.

Ten adjacent browser samples share one fixed HSLuv color. Their touching fills form a continuous spectrum with faint integer-Bark separators. Each sample falls independently. A group's single white top edge follows the tallest current sample, with opacity from the aggregate attack envelope. Intermediate frequency labels interpolate within the existing integer-Bark endpoints; they are display labels, not exact filter boundaries.

The producer consumes every loudness frame and updates a complete motion snapshot. A new visual peak restarts its 350 ms hold and white edge only when the band's input sones also rise. Gain changes can raise the current bar floor immediately but cannot restart the peak effect. The bar then follows a critically damped fall at 6/s, or 3/s with Reduced Motion. White edges use the same damped-motion curve with a shorter tail: `(1 + r*t) * exp(-r*t)`, where `t` starts at the end of the hold and `r` is 30/s, or 20/s with Reduced Motion. The glow loses 90% of its opacity in about 130 ms after the hold, or 195 ms with Reduced Motion. The hold still limits rapid repeated white attacks. A value within 0.0001 of its target settles exactly. The renderer samples that snapshot using audio time. Tests cover 30, 60, 120, 144, and 240 Hz, delayed drawing, steady levels through adaptive gain, short taps, the shorter glow tail across callback sizes, and combined bar/edge luminance reversals. These tests do not establish universal photosensitivity safety for all content and devices.

The browser runs analysis in its own AudioWorklet WASM instance. It preallocates its input and state buffers and imports no browser functions into WASM. The unified browser snapshot has 1,588 f64 values: 146 aggregate values followed by 1,442 fine values. Both headers must agree. At most one display message waits for acknowledgement; audio processing continues while the page stalls. Native and ESP capture process audio before the latest visual-state handoff. Neither live path queues raw audio for a slower renderer.
