...
for (const colorScheme of ['light', 'dark']) {
  for (const reducedMotion of ['no-preference', 'reduce']) {
    test(`group top edges follow attacks and fade over rainbow bars in ${colorScheme}, ${reducedMotion}`, async ({ page }) => {
      ...
      await page.evaluate(async () => {
        ...
        window.readBands = () => window.edgeNodes.map(node => {
          const fills = [...node.parentElement.querySelectorAll(".meter-fill")];
          const fill = fills.reduce((a, b) => a.getBoundingClientRect().top < b.getBoundingClientRect().top ? a : b);
          ...
          return {
            ...
            sideWidths: [style.borderLeftWidth, style.borderRightWidth, style.borderBottomWidth],
        ...
        window.tap = () => {
          ...
          for (let band = 0; band < 24; band++) {
            ...
            for (let local = 0; local < 10; local++) {
              state.set([.4 + band * .01 + local * .02, 0, at + .350, 1, 0, 0], 148 + (band * 10 + local) * 6);
            }
          }
      ...
      for (const band of peak) {
        ...
        expect(band.sideWidths).toEqual(['0px', '0px', '0px']);
        expect(band.shadow).toBe('none');
        expect(band.transition).toBe('0s');
        expect(band.animation).toBe('none');
      }
      await page.screenshot({ path: `test-results/top-edges-${colorScheme}-${reducedMotion}.png`, fullPage: true });
      ...
      // Fullscreen resizes the same edges; it does not scale their thickness.
      ...
      await page.screenshot({ path: `test-results/top-edges-fullscreen-${colorScheme}-${reducedMotion}.png` });
