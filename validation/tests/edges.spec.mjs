...
for (const colorScheme of ['light', 'dark']) for (const reducedMotion of ['no-preference', 'reduce']) {
  test(`canvas glow retains rainbow centers and a one-pixel inner edge in ${colorScheme}, ${reducedMotion}`, async ({ page }, info) => {
    ...
    await expect.poll(() => page.evaluate(() => {
      ...
      return v.current[v.layout[9]] / (v.height * .95);
    })).toBeGreaterThan(.39);
    const pixels = await page.evaluate(() => {
      ...
      gl.readPixels(0, Math.floor(height * .15), width, 1, gl.RGBA, gl.UNSIGNED_BYTE, values);
      return { width, ratio: v.renderer.getPixelRatio(), row: Array.from(values), colors: Array.from(v.bars.instanceColor.array), edges: Array.from(v.edges) };
    });
    expect(pixels.edges).toEqual(Array(24).fill(1));
    const rgb = x => pixels.row.slice(x * 4, x * 4 + 3);
    for (let i = 0; i < 24; i++) {
      const center = Math.floor((i + .5) * pixels.width / 24);
      const edge = (i + .02) * pixels.width / 24;
    ...
    await page.evaluate(() => { window.audioNow += .75; });
    ...
    await page.evaluate(() => window.sendBars(Array(24).fill(.4), 1));
    await expect.poll(() => page.evaluate(() => document.querySelector('#dancinglights').physics.edges[0])).toBe(1);
