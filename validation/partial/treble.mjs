...
function run(w,pcm) {
 ...
 w.processor_destroy(h); return {rows,display:stride-122};
}
...
for(const kind of ['1k-plus-8k','white','lowpass-noise','silence']){
 ...
 const mean=r=>Array.from({length:24},(_,j)=>r.rows.slice(500).reduce((sum,row)=>sum+row[r.display+2+5*j],0)/(r.rows.length-500));
